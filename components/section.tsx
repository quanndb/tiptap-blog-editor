...
import { Plus, Trash2, Columns2, Columns3 } from "lucide-react"
import type { SectionData, Column as ColumnType, Block } from "./blog-editor"
...
export function Section({ section, onUpdate, onDelete }: SectionProps) {
  const [draggedBlock, setDraggedBlock] = useState<{ block: Block; sourceColumnId: string } | null>(null)
  ...
  const updateColumn = (columnId: string, updatedColumn: ColumnType) => {
    onUpdate(section.id, {
      ...section,
      columns: section.columns.map((col) => (col.id === columnId ? updatedColumn : col)),
    })
  }
  ...
  const handleDrop = (targetColumnId: string, targetIndex?: number) => {
    ...
    const sourceColumn = section.columns.find((col) => col.id === sourceColumnId)
    ...
    const insertIndex = targetIndex !== undefined ? targetIndex : targetColumn.blocks.length
    const updatedTargetColumn = {
      ...targetColumn,
      blocks: [...targetColumn.blocks.slice(0, insertIndex), block, ...targetColumn.blocks.slice(insertIndex)],
    }
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            {section.columns.length < 3 && (
              <Button onClick={addColumn} size="sm" variant="ghost" className="h-6 w-6 p-0">
                <Plus className="w-3 h-3" />
              </Button>
            )}
            {section.columns.length === 2 && <Columns2 className="w-3 h-3 text-gray-400" />}
            {section.columns.length === 3 && <Columns3 className="w-3 h-3 text-gray-400" />}
          </div>
        </div>
        <Button
